...
defconv2D = partial(Conv2D, kernel_size=3, padding="same")
conv_encoder = Sequential([
    Conv2D(16, 3, padding="same", kernel_initializer='he_uniform', activation="relu", input_shape=[1000,1000,1]),
    MaxPool2D(pool_size=2), #output 500x500x16
    Conv2D(32, 3, padding="same", kernel_initializer='he_uniform', activation="relu"),
    MaxPool2D(pool_size=2), #output 250x250x32
    Conv2D(64, 3, padding="same", kernel_initializer='he_uniform', activation="relu"),
    MaxPool2D(pool_size=2), #output 125x125x64
    Conv2D(30, 3, padding="same", kernel_initializer='he_uniform', activation="relu"),
    GlobalAveragePooling2D() #output: 30
])

conv_decoder = Sequential([
    Dense(125 * 125 *16),
    Reshape((125, 125, 16)),
    Conv2DTranspose(32, 3, strides=2, activation="relu", kernel_initializer='he_uniform', padding="same"),
    Conv2DTranspose(16, 3, strides=2, activation="relu", kernel_initializer='he_uniform', padding="same"),
    Conv2DTranspose(1, 3, strides=2, activation="sigmoid", kernel_initializer='he_uniform', padding="same"),
    Reshape([1000,1000])
])
